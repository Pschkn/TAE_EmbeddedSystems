  \begin{tikzpicture}
	...
	\coordinate (end_a_x_mobile) at ($(begin_a_x)+(\cosAngle*\cosAngle*\sideLength,\sinAngle*\cosAngle*\sideLength)$);
	\draw[thick,green,->] (begin_a_x)  -- (end_a_x_mobile) node (x_mobile) [green,above] {$a_{x, mobile}$};
	
	\draw[thick,orange,->] (end_a_x_mobile) -- (end_a_x) node (y_mobile) [orange,above right] {$a_{y, mobile}$};
	...
	% angle vectors
	\tkzMarkAngle[fill=
	blue,size=1.2cm,opacity=.4](end_a_x,begin_a_x,end_a_x_mobile) \pic [draw, ->,
	"$\gamma$", angle eccentricity=1.5] {angle = end_a_x--begin_a_x--end_a_x_mobile};
  \end{tikzpicture}