  \begin{tikzpicture}
	...
	\draw[thick,green,->] (end_a_z_handy)  -- (end_a_y) node (y_handy)
	[green,below right] {$a_{x, handy}$};
	...
	% angle vectors
	\tkzMarkAngle[fill= green,size=1.2cm,opacity=.4](end_a_y,begin_a_z,end_a_z_handy)
    	\pic [draw, ->, "$\beta$", angle eccentricity=1.5] {angle = end_a_y--begin_a_z--end_a_z_handy};
  \end{tikzpicture}
